...
function syntax
{
  echo "The script is executed by OPSWare API to invoke the cmb_cdc_db2_restoredb.sh scripts."
  echo "syntax:"
  echo " DB2_RestoreDB.sh -HOSTNAME <HostName> -EXECUTOR <Executor> -INSTNAME <InstName> -DBNAME <DBName> -FILEPATH <FilePath> -NEWDBNAME <NewDBName> -TIMESTAMP <TimeStamp> "
  echo
  echo " where:"
  ...
  echo " Samples: "
  echo "   DB2_RestoreDB.sh -HOSTNAME dba_test -EXECUTOR db2inst1 -INSTNAME db2inst1 -DBNAME testdb1 -FILEPATH /tmp -NEWDBNAME testdb9 -TIMESTAMP <Timestamp> "
  exit 22
}
...
[ -z "${HostName}" ] && syntax [ -z "${DBName}" ] && syntax || [ -z "${InstName}" ] && syntax
...
LogFile="${WorkDir}/logs/DB2_RestoreDB.log.${StartTime}"
...
ScriptName="/home/ggroot/db2/cmb_cdc_db2_restoredb.sh"
...
ParameterString="-INSTNAME ${InstName} -DBNAME ${DBName} -FILEPATH ${FilePath} -NEWDBNAME ${NewDBName} -TIMESTAMP ${TimeStamp}"
